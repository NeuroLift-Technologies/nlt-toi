...
**Summary:** Added Natural-Language→`.toi` extraction to the TypeScript reference library (`@neurolift-technologies/toi`). New `packages/toi/src/extract.ts` (`extractToi`, `ExtractOptions`) maps confident phrase matches onto SPEC §7 fields (identity, cognitive_profile, privacy, agency, communication, ethical_pillars) deterministically — no LLM, no provider. Confident-only: weak/unmatched text never populates a field; absence = "no stated preference" (SPEC §7). Lossless: original text under `custom.freeform_terms`, extraction metadata under `custom.x-extract`. Output guaranteed conforming via `safeParseToi`. Exported through `packages/toi/src/index.ts` (semver-covered surface). Rules aligned to `templates/personal-toi-template.md` vocabulary (identifier, session-only, anonymize, research case-by-case, focused sessions, context-sensitive, etc.). 34 new tests (99 total green), `tsc --noEmit` + `vitest run` clean. **nlt-code-reviewer: APPROVE_WITH_NITS** after review-fix commit (2074df1) closed the two Important false-positive findings (call-me over-capture, bare `shared`) plus six Minor items; remaining items are non-blocking follow-ups.
**Blockers:** None.
**Next action:** Merge PR #28 to `main` (squash) after final `npm test` sanity check, then close this thread.
...
**Started:** 2026-08-01
**Last updated:** 2026-08-01
**Summary:** Phase 1 (core types + Ed25519 sign/verify) across `crates/nlt-toi` and `go/nlt-toi`, cross-validated against the TS/Python references. Added `generate_key_pair`/`GenerateKeyPair`, `signing_payload`/`SigningPayload`, `sign_toi`/`SignToi`, `verify_toi`/`VerifyToi`, `is_signed`/`IsSigned` + a dependency-free unpadded base64url codec (Rust) and `crypto/ed25519` (Go). Signature envelope corrected to `{alg, public_key, value}` per `toiSignatureSchema`. Both ports verify the committed JS-produced `signed.toi` fixture byte-for-byte and share a deterministic vector (seed 1..=32) asserting identical envelope bytes across TS/Python/Rust/Go. 30 Rust tests and 31 Go tests green; TS (65) and Python (65) reference suites unchanged. Phase gates passed: nlt-code-reviewer APPROVE_WITH_NITS (all closed in c90ea15: Go trailing-bit strictness, non-object sign guard, Debug seed redaction) and Mistral Vibe Technical Assurance APPROVE (0 critical / 0 important / 0 minor). **MERGED** to main via squash (`09a87b7`), PR #26 closed, branch deleted.
**Blockers:** None.
**Next action:** Continue Phase 2 (schema/validation + resolve) in Rust/Go, or per the port plan; run `nlt-code-reviewer` + Mistral Vibe at the next phase gate.
