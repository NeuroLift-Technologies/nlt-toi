...
1. Repo-local governance (private, primary):
   https://github.com/NeuroLift-Technologies/nlt-toi/blob/main/NLT-DEV-OTOI.md
   Public mirror (if the link above returns 404):
   https://github.com/NeuroLift-Technologies/nlt-toi/blob/main/governance/NLT-DEV-OTOI.md
2. Internal gateway (private, primary):
   https://github.com/NeuroLift-Technologies/nlt-toi/blob/main/AGENTS.md
   Public mirror (if the link above returns 404):
   https://github.com/NeuroLift-Technologies/nlt-toi/blob/main/governance/AGENTS.md
...
Format reference: `NeuroLift-Technologies/nlt-toi` templates directory.
```
...
Format reference: `templates/escalation.md` in `NeuroLift-Technologies/nlt-toi`.
```

### Step 5: Grant GitHub App Access to `nlt-toi`

Coding agents (e.g., GitHub Copilot, Codex) must be able to read the governance files in
`nlt-toi` from within the new repository. If the GitHub App installation is scoped
to **"Selected repositories"**, you must add `nlt-toi` to its access list.
...
3. Under **Repository access → Selected repositories**, add **`nlt-toi`**.
4. Click **Save**.
...
> in `nlt-toi` for full instructions.
...
- [ ] GitHub App has access to `nlt-toi` (or public mirror fallback is in place)

---

## Validation

Run the org-level validation script to check `nlt-toi` is healthy:

```bash
git clone https://github.com/NeuroLift-Technologies/nlt-toi
cd nlt-toi
