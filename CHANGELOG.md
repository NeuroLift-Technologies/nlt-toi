...
### Added
- `nlt_toi.generator` + `toi-generator` CLI — authoring surface for the Python package: `TOIDocumentGenerator` (`from_defaults`/`from_dict`, `validate`, `to_json`, `to_markdown`, `write`) produces conforming `.toi` v1.0.0 documents with privacy-first defaults (session-only retention, no sharing/training/analytics). The `toi-generator` console script supports `--interactive` (guided wizard), `--input` (JSON preferences), `--output`/`--format` (JSON or Markdown), `--validate`, and `--schema`.
- `crates/nlt-toi` and `go/nlt-toi` — **Phase 1: Ed25519 sign/verify (SPEC §11)**, cross-validated against the TS/Python references: `generate_key_pair`/`GenerateKeyPair`, `signing_payload`/`SigningPayload`, `sign_toi`/`SignToi`, `verify_toi`/`VerifyToi`, `is_signed`/`IsSigned`, plus a dependency-free unpadded base64url codec. The signature envelope is now `{alg, public_key, value}` per `toiSignatureSchema`. Both ports verify the committed JS-produced known-answer fixture (`signed.toi`) byte-for-byte, and share a deterministic Ed25519 vector (seed bytes 1..=32) asserting identical envelope bytes in Rust and Go (matching the TS/Python reference output). 13 new Rust tests (30 total) and 12 new Go tests (31 total).
